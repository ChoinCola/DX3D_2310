...
void Snow::Create()
{
    ...
    {
        Float2 size;
        size.x = MATH->Random(0.5f, 2.0f);
        size.y = size.x;

        Float3 pos;
        pos.x = MATH->Random(-buffer->GetData()->size.x, buffer->GetData()->size.x);
        pos.y = MATH->Random(-buffer->GetData()->size.y, buffer->GetData()->size.y);
        pos.z = MATH->Random(-buffer->GetData()->size.z, buffer->GetData()->size.z);

        vertices[i].pos = pos;
        vertices[i].size = size;

        vertices[i].random.x = MATH->Random(0.0f, 1.0f);
        vertices[i].random.y = MATH->Random(0.0f, 1.0f);
    }
